...
`proposal.md`, `design.md`, delta `specs/`, `tasks.md`, and `changes/<id>/test-plan.md` and
`changes/<id>/review.md` when present, the tests referenced from the test plan, the
...
2. Before calling any platform or БСП method confirm it exists and is available in the client
   ...
   `bsl_find_symbol` — a configuration may contain only part of БСП.
...
6. Run the change's tests named in the test plan when the task says so; a red test you cannot
...
- No edits of `proposal.md`, `design.md`, specs, tests or `changes/<id>/test-plan.md`.
