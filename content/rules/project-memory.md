...
1. **Recall-first.** For any non-trivial 1C task, search all connected memory providers with the task's key terms (object name, subsystem, error text) **before** solution design — same standing as `templatesearch`. On the session's first non-trivial task also query the standing conditions (`working conditions`, `benchmark`, `conventions`). Skipping an exposed memory-search provider is a defect.
